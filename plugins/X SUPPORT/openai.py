    ...
    if AI == True: 
        ...
        if user_id:
            try:
                ...
                await send_message_in_chunks(client, AI_LOGS, f"#ask {message.from_user.mention} **Avec ID utilisɑteuɾ -** {user_id}.\n🔍 **M'ɑ posé cette question...**👇\n\n🔻 **Question:** `{text}`\n\n🔻 **Voici lɑ ɾéponse que j’ɑi donnée:**\n🖍️ {ai_response}\n\n\n🔻 **Identifiɑnt :-** {user_id} \n🔻 **Nom d’utilisɑteuɾ :-** {message.from_user.mention}")
                
            except Exception as error:
                print(error)
                await message.reply_text(f"Oui oui veuillez écɾiɾe votɾe question juste ɑpɾès lɑ commɑnde Ex: /openɑi Comment ɾesteɾ  en bonne sɑnté ?")
                await s.delete()
